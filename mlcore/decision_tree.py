...
class CustomDecisionTreeClassifier:
    ...
    def _build_tree(self, X, y, depth):
        ...
        for feat in range(num_features):
            ...
            for thresh in thresholds:
                ...
                if left_mask.sum() == 0 or right_mask.sum() == 0:
                    continue
                gain = self._calc_information_gain(y, left_mask, right_mask, parent_impurity)
                if gain > best_gain:
        ...
        return DecisionNode(best_feat, best_thresh, best_gain, left_subtree, right_subtree)
    ...
    def _calc_impurity(self, y):
        assert self.criterion in ("gini", "entropy", "misclassification"), "criterion must be 'gini', 'entropy', or 'misclassification'"
    ...
    def _print_node(self, node, spacing):
        ...
        if isinstance(node, LeafNode):
            ...
            return
        print(f"{spacing}Feature[{node.feature_index}] ≤ {node.threshold:.4f}  |  Gain={node.gain:.4f}")
